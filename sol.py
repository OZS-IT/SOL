...
for j in range(1,17):
    ...
    if not path.isfile(dat):
        break
    tekmaRegistracije=j
    #Na kateri öe ne öteje za toËkovanje.
    #äe ni vkljuËeno, vljuËi enkrat.
    with open(dat,'r',encoding="utf-8") as f:
        for i in f:
            ...
            if kate[-1] == "\n":
                kate = kate[:-1]
            kat.add(kate)
            if ime=="éiga" and priimek=="Groff":
                st="483"
            elif ime == "Cesare" and priimek == "Tarrabocchia":
                klub = "OK Azimut"
            elif ime == "Andra≈æ" and priimek == "De Luisa":
                klub = "OK Azimut"
...
for st_lige in range(1,12):
    if st_lige==zadnja:#Noƒçemo, da se zgodi v sol, je pa na voljo.
        IP=1.15
